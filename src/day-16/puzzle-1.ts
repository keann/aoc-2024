...
while (paths.length > 0) {
	...
	if (nextCoords) {
		paths.unshift([nextCoords, direction, score + 1])
	}

	for (const newDirection of [rotate(direction, true), rotate(direction, false)]) {
		paths.push([coords, newDirection, score + 1000])
	}
}
